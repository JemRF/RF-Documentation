...
### Download tools
Software must be installed on the local PC's root disk drive "C:". [Download the tools here.](\firmware\Espressif.zip) The download is a compressed file **Espressif.zip**, open the file with Windows Explorer and drag the folder inside "**Espressif**" to "This PC, OS (C:).
...
Next, you need to change the location you are viewing to the location of the tools. Enter "cd / " and then do an "**ls**" as shown in Figure 2 and Figure 3 to see the files.
...
Verify you see the Espressif folder then enter "**cd .\Espressif**" followed by "**ls***" to show the list of tools are as shown in Figure 4.
...
The last step is to install the firmware on the sensor via the COM port on your computer. Enter the command "**./loader.ps COM6**" as shown in Figure 6, replacing **COM6** with the **COM** value on your PC. Note that it is case-sensitive, so use uppercase when typing "COM". If there are no issues connecting to the device, you will see the loading process start as seen in the bottom half of Figure 6.
...
### Validation 
To validate the update and it is working, follow the [Startup process](wifi32s-setup.html) to get connected to the WiFi 32s Sensor local WiFi, when working you should see Figure 9. Looking at the bottom banner you will see Version 1.1.1. Now you can now update over the air using the "**Update Available, Install?**" option to get the latest version.
