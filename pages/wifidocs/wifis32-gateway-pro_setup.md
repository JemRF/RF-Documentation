...
permalink: wifis32gateway-pro_setup.html
folder: mydoc
---
## Introduction
The WiFi Wireless Gateway Pro (WiFi Gateway Pro) has two products that have the same basic setup, the WiFi Gateway Pro and the WiFi Gateway Pro with Relay. Each receives messages from the Wireless sensors and sends them to the JemRF Monitoring service. This eliminates the need for a Raspberry PI computer and all that extra overhead. You can also connect to the our legacy service PrivateEyePi. The Gateway provides an easy to use tool to get your wireless sensors online free of configuring and programming a computer first.
...
With your Gateway now online (has a WLAN number), you can stay on your home WiFi Network and point you browser to the WLAN address of the gateway. In the example picture above the 192.168.254.21 would be the WLAN address for my Gateway, your address will change. Go to http://WLAN (example http://192.168.254.21).
#### Server
Now on the WiFi Gateway Setup Details page, the **Server** setting is  **monitoring.jemrf.com**.
{% include image.html file="wifis32gatewaypro_htu_setup.jpg" alt="WiFi GW Pro Setup"%}
...
{% include image.html file="wifis32gatewaypro_htu_list.jpg" alt="WiFi GW Sensor List"%}
